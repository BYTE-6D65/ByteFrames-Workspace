...
function BottomPanel() {
  ...
  // UI state
  const [activeTab, setActiveTab] = useState<"status" | "editor" | "logs">("editor")
  ...
  useEffect(() => {
    const init = async () => {
      try {
        ...
        const oldConfigs = JSON.parse(localStorage.getItem("obs_overlay_configs") || "[]")
        ...
        if (dbConfigs.length > 0) {
          ...
          initialActiveConfigId = dbConfigs.find(c => c.id === lastActiveConfigId)?.id || dbConfigs[0].id;
  ...
  const selectedWidget = activeConfig?.widgets.find(w => w.id === selectedWidgetId) || null
  ...
  const handleSelectWidget = async (widget: Widget) => {
    if (isDirty) {
      const shouldSwitch = confirm("You have unsaved changes. Switch widgets anyway?")
  ...
  const handleSaveWidget = async () => {
    ...
    if (defaultNamePattern.test(selectedWidget.name)) {
      const newName = prompt("Please give this widget a meaningful name before saving:", selectedWidget.name)
      if (!newName) return
      if (newName.trim() === "") {
  ...
  const applyOverlay = async () => {
    ...
    try {
      ...
      const { loadMultipleWidgets } = await import("./overlay/multiWidgetRuntime")
  ...
  const clearOverlay = async () => {
    try {
      const { unmountAllWidgets } = await import("./overlay/multiWidgetRuntime")
      unmountAllWidgets()
      setApplied("")
  ...
  return (
    <div className="bf-panel" style={{ display: "flex", flexDirection: "column", height: "100%" }}>
      {/* Tab buttons */}
      <div style={{ display: "flex", gap: "4px", padding: "8px 8px 0 8px", borderBottom: "1px solid #1f2533" }}>
        <button
      ...
      {/* Tab content */}
      <div className="bf-panel-body" style={{ flex: 1, overflow: "hidden", padding: activeTab === "editor" ? "8px" : "0.5rem 0.6rem", display: "flex", flexDirection: "column" }}>
        {activeTab === "status" && <StatusPanel />}
        {activeTab === "editor" && (
          <div style={{ display: "grid", gridTemplateColumns: "70fr 30fr", gap: "8px", flex: 1, minHeight: 0 }}>
            ...
            {!selectedWidget ? (
              ...
              <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: "8px", minHeight: 0 }}>
                ...
                <div style={{ display: "flex", flexDirection: "column", gap: "4px" }}>
                  <div style={{ fontSize: "0.75rem", color: "#a8c3ff", fontWeight: 600 }}>
                    JavaScript {isDirty && <span style={{ color: "#ff9f40" }}>●</span>}
                  </div>
                  <div style={{ flex: 1, border: "1px solid #1f2533", borderRadius: "8px", overflow: "hidden" }}>
                    <MonacoEditor
                      language="javascript"
                      value={workingJs}
  ...
  return (
    <div className="bf-video-shell">
      ...
      <div className={`bf-video-fallback ${status === "live" ? "hidden" : ""}`}>
        ...
        <div className="bf-kv-value">Use OBS VirtualCam/NDI to route program output here.</div>
...
function VideoBindingControls() {
  ...
  const stop = () => {
    ...
    setStatus("idle")
    document.querySelectorAll<HTMLVideoElement>("video[data-obs-preview=true]").forEach((el) => {
      el.srcObject = null
    })
  }
  ...
  return (
    <div className="obs-card">
      ...
      <div className="obs-card-row tight">
        <select className="obs-input" value={selected} onChange={(e) => setSelected(e.target.value)}>
          {devices.map((d) => (
        ...
        <button className="obs-btn" onClick={bind} disabled={status === "binding"}>
          {status === "live" ? "Rebind" : "Bind"}
        </button>
        <button className="obs-btn ghost" onClick={stop} disabled={status === "idle"}>
          Stop
        </button>
      </div>
